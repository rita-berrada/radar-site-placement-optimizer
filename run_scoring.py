#!/usr/bin/env python3
import numpy as np
from score import score_one_radar
...
def main():
    ...
    # -----------------------------------
    results = []  # list of (score, lat, lon, cov_by_fl)

    for k in range(K):
        lat, lon = points[k]
        score, cov_by_fl = score_one_radar(
            radar_lat=float(lat),
            radar_lon=float(lon),
        ...
        results.append((float(score), float(lat), float(lon), cov_by_fl))
        print(f"[{k+1}/{K}] lat={lat:.1f}, lon={lon:.1f} -> score={score:.4f}")
    ...
    for rank, (s, lat, lon, cov) in enumerate(results[:10], start=1):
        print(f"{rank:>2}. lat={lat:.1f}, lon={lon:.1f} | score={s:.4f} | cov_by_FL={cov}")
    ...
    np.savez("scored_candidates_01deg.npz", scores_latlon=scores_arr, cov_by_fl=cov_arr)
    print("\n✓ Saved results to scored_candidates_01deg.npz")
